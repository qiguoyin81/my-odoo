...
import './ManufacturingOrder.less';
...
const ManufacturingOrder = () => {
  const [selectedView, setSelectedView] = useState('pivot');
  ...
  const [selectedFilter, setSelectedFilter] = useState('待办');
  ...
  const viewOptions = [
    ...
    { key: 'pivot', label: '透视表', icon: <TableOutlined /> },
  ...
  const PivotView = () => {
    // 透视表数据
    const pivotData = [
      { product: '[FURN_7800] 组合桌子', draft: 0, toClose: 1, inProgress: 0, completed: 0, total: 1 },
      { product: '[FURN_8000] 办公椅', draft: 1, toClose: 0, inProgress: 0, completed: 0, total: 1 },
      { product: '[FURN_8200] 会议桌', draft: 0, toClose: 1, inProgress: 0, completed: 0, total: 1 },
      { product: '[FURN_8500] 文件柜', draft: 1, toClose: 0, inProgress: 0, completed: 0, total: 1 },
      { product: '总计', draft: 2, toClose: 2, inProgress: 0, completed: 0, total: 4 }
    ];
    ...
    return (
      <Card className="pivot-container">
        <Tabs defaultActiveKey="1" className="pivot-tabs">
          <TabPane tab="按产品分组" key="1">
            <Table
              columns={columns}
          ...
          <TabPane tab="按状态分组" key="2">
            <Table
              columns={[
                { title: '状态', dataIndex: 'status', key: 'status', fixed: 'left' },
                { title: '草稿', dataIndex: 'draft', key: 'draft', align: 'right' },
                { title: '待关闭', dataIndex: 'toClose', key: 'toClose', align: 'right' },
                { title: '进行中', dataIndex: 'inProgress', key: 'inProgress', align: 'right' },
                { title: '已完成', dataIndex: 'completed', key: 'completed', align: 'right' },
                { title: '总计', dataIndex: 'total', key: 'total', align: 'right', fixed: 'right' }
              ]}
              dataSource={[
                { status: '草稿', draft: 2, toClose: 0, inProgress: 0, completed: 0, total: 2 },
                { status: '待关闭', draft: 0, toClose: 2, inProgress: 0, completed: 0, total: 2 },
                { status: '总计', draft: 2, toClose: 2, inProgress: 0, completed: 0, total: 4 }
              ]}
              pagination={false}
              scroll={{ x: 'max-content' }}
              className="pivot-table"
          ...
          <TabPane tab="按负责人分组" key="3">
            <Table
              columns={[
                { title: '负责人', dataIndex: 'responsible', key: 'responsible', fixed: 'left' },
                { title: '草稿', dataIndex: 'draft', key: 'draft', align: 'right' },
                { title: '待关闭', dataIndex: 'toClose', key: 'toClose', align: 'right' },
                { title: '进行中', dataIndex: 'inProgress', key: 'inProgress', align: 'right' },
                { title: '已完成', dataIndex: 'completed', key: 'completed', align: 'right' },
              ...
              dataSource={[
                { responsible: '张三', draft: 0, toClose: 1, inProgress: 0, completed: 0, total: 1 },
                { responsible: '李四', draft: 1, toClose: 0, inProgress: 0, completed: 0, total: 1 },
                { responsible: '王五', draft: 0, toClose: 1, inProgress: 0, completed: 0, total: 1 },
                { responsible: '赵六', draft: 1, toClose: 0, inProgress: 0, completed: 0, total: 1 },
                { responsible: '总计', draft: 2, toClose: 2, inProgress: 0, completed: 0, total: 4 }
              ]}
              pagination={false}
              scroll={{ x: 'max-content' }}
              className="pivot-table"
